...
namespace eosio {
   ...
   public:
      ...
      void accepted_block(const block_id_type& block_id, const uint32_t& block_num, const signed_block_ptr& block_ptr);
      ...
      void expire_local_txns(uint32_t);
...
#define peer_elog(PEER, FORMAT, ...) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   public:
      ...
      unique_ptr<boost::asio::steady_timer> write_out_timer;
      ...
      shared_ptr<vector<char>> pack_signed_block(const signed_block_ptr& sb);
   ...
   public:
      ...
      void bcast_block(const block_id_type& block_id, const uint32_t& block_num, const signed_block_ptr& blocks);
   ...
   void connection::initialize()
   {
      ...
      write_out_timer.reset(new boost::asio::steady_timer(net_plugin::get_io_service()));
   }
   ...
   void connection::close()
   {
      ...
      if (write_out_timer) write_out_timer->cancel();
   }
   ...
   void connection::blk_send_branch()
   {
      ...
      boost::asio::post(app().get_io_service(), [wpconn]() {
         ...
         boost::asio::post(net_plugin::get_io_service(), [wpconn, head_num, temp_head_id, temp_lib_id]() {
            ...
            if (conn->peer_requested->start_block<=conn->peer_requested->end_block) {
               fc_dlog(logger, "enqueue ${s} - ${e}", ("s", conn->peer_requested->start_block)
                  ("e", conn->peer_requested->end_block));
   ...
   void connection::fill_out_buffer_with_pbft_queue(std::vector<boost::asio::const_buffer>& bufs)
   {
      ...
      std::function<void(boost::system::error_code, std::size_t)> callback = [weak_this, close_after_send](
   ...
   shared_ptr<vector<char>> connection::pack_signed_block(const signed_block_ptr& sb)
   {
...
//            SaveTestCase(hmsg, p.c_str(), FileName);
   ...
   sync_manager::sync_manager(uint32_t req_span)
      :sync_known_lib_num(0), sync_last_requested_num(0), sync_next_expected_num(1), sync_req_span(req_span), source(),
       state(in_sync)
   {
   ...
   {
      boost::asio::post(app().get_io_service(), [this, conn]() {
         uint32_t head_block = chain_plug->chain().fork_db_head_block_num();
         auto temp_sync_known_lib_num = chain_plug->chain().last_irreversible_block_num();

         boost::asio::post(net_plugin::get_io_service(), [this, head_block, temp_sync_known_lib_num, conn]() {
            ...
            fc_dlog(logger, "request, head is ${h} last req = ${r}", ("h", head_block)("r", sync_last_requested_num));
            /* ----------
            ...
            if (!source || !source->current()) {
               ...
               sync_known_lib_num = temp_sync_known_lib_num;
               sync_last_requested_num = 0;
   ...
   bool sync_manager::sync_required(uint32_t fork_db_head_block_num)
   {
      ...
      return (sync_last_requested_num<sync_known_lib_num ||
         fork_db_head_block_num<sync_last_requested_num);
   }

   void sync_manager::start_sync(const connection_ptr& c, uint32_t target)
   {
      boost::asio::post(app().get_io_service(), [this, c, target]() {
         auto& cc = my_impl->chain_plug->chain();
         if (target>sync_known_lib_num) sync_known_lib_num = target;

         uint32_t hnum = cc.fork_db_head_block_num();
         uint32_t bnum = cc.last_irreversible_block_num();
         if (!sync_required(hnum) || target<=bnum) {
         ...
         if (state==in_sync) {
            ...
            sync_next_expected_num = cc.last_irreversible_block_num()+1;
         }
   ...
   void sync_manager::recv_handshake(const connection_ptr& c, const handshake_message& msg)
   {
      boost::asio::post(app().get_io_service(), [this, c, msg]() {
         ...
         boost::asio::post(net_plugin::get_io_service(), [this, c, msg, lib_num, head, head_id, on_fork]() {
   ...
   void dispatch_manager::bcast_block(const block_id_type& block_id, const uint32_t& block_num,
      const signed_block_ptr& block_ptr)
   {

      std::set<connection_ptr> skips;
      auto range = received_blocks.equal_range(block_id);
      for (auto org = range.first; org!=range.second; ++org) {
         skips.insert(org->second);
      }
      received_blocks.erase(range.first, range.second);
      peer_block_state pbstate = {block_id, block_num, false, true, time_point()};
      ...
      for (auto& cp : my_impl->connections) {
         ...
         bool has_block = cp->last_handshake_recv.last_irreversible_block_num>=block_num;
         if (!has_block) {
            fc_dlog(logger, "bcast block ${b} to ${p}", ("b", block_num)("p", cp->peer_name()));
            cp->add_peer_block(pbstate);
            cp->enqueue_block(block_ptr);
         }
      }
   ...
   {
      if (msg.known_trx.mode!=normal && msg.known_trx.mode!=none) {
         elog("passed a notice_message with something other than a normal on none known_trx");
         return;
      }
      if (msg.known_blocks.mode!=normal && msg.known_blocks.mode!=none) {
      ...
      tcp::resolver::results_type endpoints)
   {
      ...
      boost::asio::async_connect(*c->socket, endpoints,
         [weak_conn, resolver, socket = c->socket, this](const boost::system::error_code& err,
   ...
   {
      fc_dlog(logger, "start_read_message.");
      try {
         if (!conn->socket) {
            return;
         }
         ...
         if (conn->buffer_queue.write_queue_size()>def_max_write_queue_size ||
            ...
            if (conn->buffer_queue.write_queue_size()>2*def_max_write_queue_size ||
               ...
               fc_elog(logger, "  max trx in progress ${s} bytes", ("s", sz));
               my_impl->close(conn);
               return;
            }
                        ...
                        if (bytes_in_buffer<message_header_size) {
                           ...
                           if (message_length>def_send_buffer_size*2 || message_length==0) {
                              ...
                              elog("incoming message length unexpected (${i}), from ${p}",
                                 ("i", message_length)("p",
                                    boost::lexical_cast<std::string>(conn->socket->remote_endpoint(ec))));
                              close(conn);
                              return;
                           }
                           ...
                           if (bytes_in_buffer>=total_message_bytes) {
                              ...
                              if (outstanding_message_bytes>available_buffer_bytes) {
                                 conn->pending_message_buffer.add_space(
                                    outstanding_message_bytes-available_buffer_bytes);
                              }
   ...
   {
      fc_dlog(logger, "handle_message, chain_size_message");
   ...
   {
      fc_dlog(logger, "handle_message, request_p2p_message");
   ...
   {
      fc_dlog(logger, "handle_message, response_p2p_message");
      auto peer_record = p2p_peer_records.find(c->peer_addr);
      if (peer_record!=p2p_peer_records.end()) {
   ...
   {
      fc_dlog(logger, "handle_message, handshake_message");
      if (!is_valid(msg)) {
      ...
      if (msg.generation==1) {
         ...
         boost::asio::post(app().get_io_service(), [this, peer_lib, msg, c]() {
            uint32_t lib_num = chain_plug->chain().last_irreversible_block_num();
   ...
   {
      fc_dlog(logger, "handle_message, time_message");
      ...
      if (logger.is_enabled(fc::log_level::all))
         logger.log(
            FC_LOG_MESSAGE(all, "Clock offset is ${o}ns (${us}us)", ("o", c->offset)("us", c->offset/NsecPerUsec)));
      c->org = 0;
      c->rec = 0;
   }
   ...
   void net_plugin_impl::handle_message(const connection_ptr& c, const request_message& msg)
   {
      ...
      case normal :
         ...
         if (!msg.req_blocks.ids.empty()) {
            fc_dlog(logger, "received request_message, sending ${num} blocks from my node",
               ("num", msg.req_blocks.ids.size()));
   ...
   {
      fc_dlog(logger, "handle_message, checkpoint_request_message");
      ...
      boost::asio::post(app().get_io_service(), [this, c, msg]() {
         ...
         shared_ptr<list<pbft_stable_checkpoint>> scp_stack = make_shared<list<pbft_stable_checkpoint>>();
         controller& cc = my_impl->chain_plug->chain();
         ...
         if (!scp_stack->empty())
            fc_dlog(logger, "sending ${n} stable checkpoints on my node", ("n", scp_stack->size()));
   ...
   {
      fc_dlog(logger, "handle_message, packed_transaction_ptr");
      fc_dlog(logger, "got a packed transaction, cancel wait");
      peer_ilog(c, "received packed_transaction");
      boost::asio::post(app().get_io_service(), [this, c, trx]() {
         controller& cc = my_impl->chain_plug->chain();
         ...
         boost::asio::post(net_plugin::get_io_service(), [this, c, trx] {
            ...
            boost::asio::post(app().get_io_service(), [c, this, ptrx]() {
               chain_plug->accept_transaction(ptrx,
                  [c, this, ptrx](const static_variant<fc::exception_ptr, transaction_trace_ptr>& result) {
                     c->trx_in_progress_size -= calc_trx_size(ptrx->packed_trx);
                     if (result.contains<fc::exception_ptr>()) {
                        fc_dlog(logger, "bad packed_transaction : ${m}",
                           ("m", result.get<fc::exception_ptr>()->what()));
                     }
                     else {
   ...
   {
      fc_dlog(logger, "handle_message, signed_block_ptr");
      fc_dlog(logger, "canceling wait on ${p}", ("p", c->peer_name()));
      c->cancel_wait();
      auto accept_pbft_stable_checkpoint = [this, c, msg]() {
         // async execute int main thread;
         auto block_num = chain_plug->chain().last_stable_checkpoint_block_num();
      ...
      boost::asio::post(app().get_io_service(), [this, c, msg, accept_pbft_stable_checkpoint]() {
         // async run in main thread
         ...
         boost::asio::post(net_plugin::get_io_service(), [this, c, blk_id, blk_num, msg]() {
         ...
         try {
            ...
            peer_elog(c, "bad signed_block : ${m}", ("m", ex.what()));
            elog("unable to accept block on assert exception ${n} from ${p}",
               ("n", ex.to_string())("p", c->peer_name()));
         }
         catch (const fc::exception& ex) {
            peer_elog(c, "bad signed_block : ${m}", ("m", ex.what()));
            elog("accept_block threw a non-assert exception ${x} from ${p}",
         ...
         boost::asio::post(net_plugin::get_io_service(), [this, reason, msg, c]() {
            block_id_type blk_id = msg->id();
            uint32_t blk_num = msg->block_num();
...
//        if (sync_master->is_syncing()) return;
   ...
   {
      fc_dlog(logger, "handle_message, pbft_prepare");
      if (is_pbft_msg_valid(msg) && maybe_add_to_pbft_cache(std::string(msg.sender_signature))) {

         boost::asio::post(app().get_io_service(), [this, msg, c]() {
            auto pmm = make_shared<pbft_message_metadata<pbft_prepare>>(msg, chain_id);
            ...
            boost::asio::post(get_io_service(), [this, pmm, c, sender_key]() {
            ...
            pbft_incoming_prepare_channel.publish(pmm);
         });
      }
   ...
   {
      fc_dlog(logger, "handle_message, pbft_commit");
      if (is_pbft_msg_valid(msg) && maybe_add_to_pbft_cache(std::string(msg.sender_signature))) {
         auto pmm = make_shared<pbft_message_metadata<pbft_commit>>(msg, chain_id);

         boost::asio::post(app().get_io_service(), [this, pmm, c, msg]() {
            pmm->cal_sender_key(my_impl->chain_plug->chain().get_thread_pool(), chain_id);
            pbft_controller& pcc = my_impl->chain_plug->pbft_ctrl();
            auto sender_key = pmm->get_sender_key();
            ...
            pbft_incoming_commit_channel.publish(pmm);
         });
   ...
   {
      fc_dlog(logger, "handle_message, pbft_view_change");
      if (is_pbft_msg_valid(msg) && maybe_add_to_pbft_cache(std::string(msg.sender_signature))) {
         boost::asio::post(app().get_io_service(), [this, msg, c]() {
            shared_ptr<pbft_message_metadata<pbft_view_change>> pmm =
               make_shared<pbft_message_metadata<pbft_view_change>>(msg, chain_id);
            pmm->cal_sender_key(my_impl->chain_plug->chain().get_thread_pool(), chain_id);
            auto sender_key = pmm->get_sender_key();
            pbft_controller& pcc = my_impl->chain_plug->pbft_ctrl();
            ...
            shared_ptr<set<block_id_type>> missing_blocks = make_shared<set<block_id_type>>();
            for (auto const b: pmm->msg.prepared_cert.pre_prepares) {
               if (!ctrl.fetch_block_by_id(b)) missing_blocks->emplace(b);
            }

            boost::asio::post(net_plugin::get_io_service(), [this, missing_blocks, c, pmm, sender_key]() {
            ...
            pbft_incoming_view_change_channel.publish(pmm);
         });
      }
   }
   ...
   {
      fc_dlog(logger, "handle_message, pbft_new_view");
      if (maybe_add_to_pbft_cache(std::string(msg.sender_signature))) {
         boost::asio::post(app().get_io_service(), [this, c, msg]() {
            pbft_controller& pcc = my_impl->chain_plug->pbft_ctrl();
            ...
            shared_ptr<pbft_message_metadata<pbft_new_view>> pmm =
               make_shared<pbft_message_metadata<pbft_new_view>>(msg, chain_id);
            pmm->cal_sender_key(my_impl->chain_plug->chain().get_thread_pool(), chain_id);
            auto sender_key = pmm->get_sender_key();
            if (sender_key!=pcc.pbft_db.get_new_view_primary_key(pmm->msg.new_view))
               return;

            boost::asio::post(net_plugin::get_io_service(), [this, pmm, c, sender_key]() {
               bcast_pbft_msg(pmm->msg, 60*pbft_message_TTL, c);
               fc_dlog(logger, "received new view: ${n}, from ${v}", ("n", pmm->msg)("v", sender_key));
            });

            pbft_incoming_new_view_channel.publish(pmm);
         });
      }
   }
   ...
   {
      fc_dlog(logger, "handle_message, compressed_pbft_message");
   ...
   {
      fc_dlog(logger, "handle_message, pbft_checkpoint");
      if (is_pbft_msg_valid(msg) && maybe_add_to_pbft_cache(std::string(msg.sender_signature))) {
         boost::asio::post(app().get_io_service(), [this, c, msg]() {
            shared_ptr<pbft_message_metadata<pbft_checkpoint>> pmm =
               make_shared<pbft_message_metadata<pbft_checkpoint>>(msg, chain_id);
            ...
            boost::asio::post(net_plugin::get_io_service(), [this, c, msg, pmm, sender_key]() {
               bcast_pbft_msg(pmm->msg, pbft_message_TTL, c);
               fc_dlog(logger, "received checkpoint at ${n}, from ${v}",
                  ("n", pmm->msg.block_info.block_num())("v", sender_key));
            });

            pbft_incoming_checkpoint_channel.publish(pmm);
         });
      }
   }
   ...
   {
      fc_dlog(logger, "handle_message, pbft_stable_checkpoint");
      boost::asio::post(app().get_io_service(), [this, c, msg]() {
   ...
   void net_plugin_impl::expire_txns()
   {
      boost::asio::post(app().get_io_service(), [this]() {
         uint32_t lib = chain_plug->chain().last_irreversible_block_num();
         boost::asio::post(net_plugin::get_io_service(), [this, lib]() {
            start_txn_timer();
            auto now = time_point::now();
            auto start_size = local_txns.size();
            expire_local_txns(lib);
   ...
   void net_plugin_impl::expire_local_txns(uint32_t lib)
   {
   ...
   void net_plugin_impl::accepted_block(const block_id_type& block_id, const uint32_t& block_num,
      const signed_block_ptr& block_ptr)
   {
      fc_dlog(logger, "signaled, id = ${id}", ("id", block_id));
      dispatcher->bcast_block(block_id, block_num, block_ptr);
   }
   ...
   boost::asio::io_service& net_plugin::get_io_service()
   {
      return net_plugin_impl::get_io_service();
   }

   net_plugin_impl::net_plugin_impl()
      :
      pbft_incoming_prepare_channel(
         app().get_channel<eosio::chain::plugin_interface::pbft::incoming::prepare_channel>()),
      ...
      pbft_incoming_view_change_channel(
         app().get_channel<eosio::chain::plugin_interface::pbft::incoming::view_change_channel>()),
      pbft_incoming_new_view_channel(
         app().get_channel<eosio::chain::plugin_interface::pbft::incoming::new_view_channel>()),
      pbft_incoming_checkpoint_channel(
         app().get_channel<eosio::chain::plugin_interface::pbft::incoming::checkpoint_channel>()) { }
   ...
   void net_plugin_impl::subthread_shutdown()
   {
      try {
         ...
         if (connector_check)
            connector_check->cancel();
         if (transaction_check)
            transaction_check->cancel();
         if (keepalive_timer)
         ...
         if (acceptor) {
            ...
            ilog("close ${s} connections", ("s", connections.size()));
