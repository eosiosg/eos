    ...
    namespace chain {
        ...
        void pbft_database::add_pbft_prepare(pbft_prepare &p) {
            ...
            while ((prev != nullptr) && (prev->block_num > ctrl.last_irreversible_block_num())) {
                ...
                auto prev_p = p;
                auto prev_ps = pbft_state{prev->id, prev->block_num, {prev_p}};
                ...
                if (prev_itr == by_block_id_index.end()) {
                    ...
                    auto p_itr = find_if(prepares.begin(), prepares.end(),
                            [&](const pbft_prepare &prep) { return prep.public_key == p.public_key; });
                    if (p_itr == prepares.end() || p_itr->view != p.view) {
                        by_block_id_index.modify(prev_itr, [&](const pbft_state_ptr &psp) {
                            psp->prepares.emplace_back(prev_p);
        ...
        void pbft_database::add_pbft_commit(pbft_commit &c) {
            ...
            while ((prev != nullptr) && (prev->block_num > ctrl.last_irreversible_block_num())) {
                ...
                auto prev_itr = by_block_id_index.find(prev->id);
                auto prev_c = c;
                auto prev_ps = pbft_state{prev->id, prev->block_num, .commits={prev_c}};
                auto prev_psp = make_shared<pbft_state>(prev_ps);
                ...
                if (prev_itr == by_block_id_index.end()) {
                    ...
                    auto p_itr = find_if(commits.begin(), commits.end(),
                            [&](const pbft_commit &comm) { return comm.public_key == c.public_key; });
                    if (p_itr == commits.end() || p_itr->view != c.view) {
                        by_block_id_index.modify(prev_itr, [&](const pbft_state_ptr &psp) {
                            psp->commits.emplace_back(prev_c);
        ...
        uint32_t pbft_database::get_committed_view() {
            ...
            auto as = blk->active_schedule;
            flat_map<uint32_t,uint32_t> commit_count;
            for (const auto &com: commits) {
                if (commit_count.find(com.view) == commit_count.end()) commit_count[com.view] += 1;
            }

            for (auto const &e: commit_count) {
                if (e.second >= blk->active_schedule.producers.size() * 2 / 3 + 1 && e.first > new_view) {
                    new_view = e.first;
                }
            }
        ...
        uint32_t pbft_database::should_view_change() {
            ...
            if (itr == by_count_and_view_index.end()) return nv;
                ...
                uint32_t new_view)
        {
            if (!vcv.empty()) {
                for (auto vc : vcv) {
                    ...
                    ilog("retry pbft outgoing view change msg: ${v} uuid: ${uuid}",("v", vc.view)("uuid", vc.uuid));
                ...
                uint32_t current_view)
        {
            ...
            ilog("sending new view ${nv}", ("nv", nv));
            return nv;
        }
        ...
        vector<pbft_prepared_certificate> pbft_database::generate_prepared_certificate() {
            ...
            const auto &by_commit_and_num_index = index.get<by_prepare_and_num>();
            auto itr = by_commit_and_num_index.begin();
            if (itr == by_commit_and_num_index.end()) return vector<pbft_prepared_certificate>{};
            pbft_state_ptr psp = *itr;
        ...
        vector<pbft_committed_certificate> pbft_database::generate_committed_certificate() {
            ...
            if (psp->should_committed && (psp->block_num > (ctrl.last_irreversible_block_num()))) {
                for (auto const &my_sp : ctrl.my_signature_providers()) {
                    ...
                    auto pc = pbft_committed_certificate{psp->block_id, psp->block_num, valid_commits, my_sp.first};
                    pc.producer_signature = my_sp.second(pc.digest());
                    pcc.emplace_back(pc);
                }
        ...
        vector<pbft_view_changed_certificate> pbft_database::generate_view_changed_certificate() {
            ...
            if (pvs->should_view_changed) {
                ...
                ilog("generating view change cert ${pc}", ("pc", vcc));
                return vcc;
...
//                   && certificate.prepared.is_signature_valid()
//                   && is_valid_prepared_certificate(certificate.prepared)
//                   && certificate.committed.is_signature_valid()
...
//            if (pbft_ctrl.current_view >= certificate.view) return false;
        ...
        pbft_stable_checkpoint pbft_database::get_stable_checkpoint_by_id(const block_id_type &block_id) {
            auto sc = vector<pbft_stable_checkpoint>{};
...
//                auto during_bp_change = in - ctrl.last_promoted_proposed_schedule_block_num();
        ...
        void pbft_database::add_pbft_checkpoint(pbft_checkpoint &cp) {
            ...
            if (!csp->is_stable) {
                ...
                if (cp_count >= active_bps.size() * 2 / 3 + 1) {
                    ...
                    ilog("checkpoint at ${h} turns stable", ("h", csp->block_num));
                }
            }

            auto pending_num = cal_latest_possible_stable_checkpoint_block_num();
            ilog("pending checkpoint ${h}", ("h", pending_num));
        ...
        bool pbft_database::is_valid_checkpoint(const pbft_checkpoint &cp) {
            return cp.is_signature_valid();
        }

        bool pbft_database::is_valid_stable_checkpoint(const pbft_stable_checkpoint &cp) {
            bool valid = false;
            if (ctrl.fetch_block_state_by_id(cp.block_id) != block_state_ptr()) {
                auto active_bps = ctrl.fetch_block_state_by_number(cp.block_num)->active_schedule.producers;
                valid = cp.is_signature_valid();
                for (auto c: cp.checkpoints) {
                    valid = c.is_signature_valid() && c.block_id == cp.block_id && c.block_num == cp.block_num;
                    if (!valid) break;
                }
            }
            return valid;
